	...
	override async generateAuthUrl(state: string): Promise<{ url: string; instructions?: string }> {
		...
		return {
			...
			instructions:
				...
				"If the redirect opens VS Code instead of returning to Pi, copy the full " +
				"vscode://gitlab.gitlab-workflow/authentication?... callback URL from VS Code/browser and paste it back into Pi.",
		};
	}
