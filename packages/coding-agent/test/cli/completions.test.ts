...
describe("generateCompletion — bash", () => {
	...
	it("registers the dispatcher and resolves alias arms to the canonical handler", () => {
		expect(out).toContain("complete -F _pi pi");
		expect(out).toContain("_pi_cmd_commit");
	...
	it("completes enum, dynamic, and comma-list flag values by previous flag", () => {
		...
		expect(out).toContain('--model)\n\t\t\tCOMPREPLY=( $(compgen -W "$(command pi __complete models -- "$cur"');
		expect(out).toContain("--resume|-r)");
		expect(out).toContain("command pi __complete sessions");
	...
	it("completes a subcommand's positional enum and its own flags", () => {
		expect(out).toContain("_pi_cmd_worktree()");
...
describe("generateCompletion — zsh", () => {
	...
	it("emits the compdef header and dual-mode (autoload + eval) tail", () => {
		expect(out.startsWith("#compdef pi")).toBe(true);
		expect(out).toContain('if [ "$funcstack[1]" = "_pi" ]; then');
		expect(out).toContain("compdef _pi pi");
	});

	it("maps value sources to the right _arguments actions", () => {
		expect(out).toContain("'--model[Model to use]:model:_pi_call models'");
		expect(out).toContain("'--models[Model list]:models:_pi_models_list'");
		expect(out).toContain("'--thinking[Effort]:value:(low high)'");
		expect(out).toContain("'--tools[Tools]:value:_pi_tools'");
		expect(out).toContain("'(-r --resume)'{-r,--resume}'[Resume]:session:_pi_call sessions'");
		...
		expect(out).toContain("_pi_tools() { _values -s , 'tools' read bash }");
	});

	it("dispatches aliased subcommands and completes positional enums", () => {
		expect(out).toContain("worktree|wt) _pi_cmd_worktree ;;");
...
describe("generateCompletion — fish", () => {
	...
	it("declares the no-subcommand predicate over every command token", () => {
		expect(out).toContain("function __fish_pi_no_subcommand");
	...
	it("maps value sources to fish completion args", () => {
		expect(out).toContain("-l model -d 'Model to use' -x -a '(command pi __complete models -- (commandline -ct))'");
		expect(out).toContain("-l thinking -d 'Effort' -x -a 'low high'");
		expect(out).toContain("-l tools -d 'Tools' -x -a 'read bash'");
		expect(out).toContain("-s r -l resume -d 'Resume' -x -a '(command pi __complete sessions");
...
describe("pi completions (integration / drift)", () => {
	it("emits a zsh script reflecting the live command + flag surface", async () => {
		...
		expect(stdout).toContain(":value:(off auto minimal low medium high xhigh max ultra)");
		expect(stdout).toContain(":value:(always-ask write yolo)");
		// Real retained subcommands present; dynamic callbacks wired.
		expect(stdout).toContain("_pi_cmd_plugin");
		expect(stdout).toContain("'completions:");
		// zsh routes single-value dynamic flags through the _pi_call action, which
		// itself shells out to `pi __complete $kind`.
		expect(stdout).toContain("_pi_call models");
		expect(stdout).toContain("_pi_call sessions");
		expect(stdout).toContain("command pi __complete $kind");
		// Hidden/default commands must NOT surface as completable subcommands.
		expect(stdout).not.toContain("_pi_cmd_launch");
		expect(stdout).not.toContain("_pi_cmd___complete");
		expect(stdout).not.toContain("--no-rules[");
		for (const disabledCommand of ["commit", "update", "acp", "join", "ssh", "say", "ttsr", "auth_broker"]) {
			expect(stdout).not.toContain(`_pi_cmd_${disabledCommand}`);
		}
		expect(stdout).not.toMatch(/(?:^|[^a-z])omp(?:[^a-z]|$)|\.omp|omp:\/\//i);
	});
});
