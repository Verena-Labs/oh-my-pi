...
describe("createAgentSession deferred model pattern resolution", () => {
	...
	test("does not install deferred bare role fallback chains", async () => {
		const settings = Settings.isolated();
	...
	test("does not install an inherited fallback chain for a deferred singleton modelPattern", async () => {
		const settings = Settings.isolated({
		...
		try {
			expect(session.model?.provider).toBe("runtime-provider");
			expect(session.model?.id).toBe("runtime-model");
			expect(session.settings.getModelRole("subagent:deferred-default")).toBeUndefined();
			expect(session.settings.get("retry.fallbackChains")["subagent:deferred-default"]).toBeUndefined();
		} finally {
			await session.dispose();
		}
	});
