...
import { APP_NAME, isEexist, isEnoent, logger, postmortem, procmgr, sanitizeText } from "@oh-my-pi/pi-utils";
...
export const DAEMON_BROKER_PROCESS_TITLE = `${APP_NAME} daemon broker`;
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!lease) return;
	process.title = DAEMON_BROKER_PROCESS_TITLE;
