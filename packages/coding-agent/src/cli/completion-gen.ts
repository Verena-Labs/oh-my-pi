...
function buildFlags(Cmd: CommandCtor): CompletionFlag[] {
	...
	for (const name in flags) {
		...
		out.push({
			name,
			char: desc.char,
			description: normalizePublicCliText(desc.description ?? ""),
...
function buildArgs(Cmd: CommandCtor): CompletionArg[] {
	...
	for (const name in args) {
		...
		out.push({ name, description: normalizePublicCliText(desc.description ?? ""), value: argValue(desc) });
	}
	return out;
}
...
export function buildSpec(
	...
	for (const [name, Cmd] of config.commands) {
		...
		commands.push({
			...
			description: normalizePublicCliText(Cmd.description ?? ""),
			flags,
			args,
		});
	}
...
function bashValueBranch(bin: string, v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return `_pi_comma "${bashWords(v.values)}"; return 0`;
		case "models":
			return v.multiple
				? `_pi_comma "$(command ${bin} __complete models 2>/dev/null | cut -f1)"; return 0`
	...
	for (const c of spec.commands) {
		...
		const elseBranch = argWords
			...
			: fileArg
				...
				: ":";
		parts.push(`_pi_cmd_${bashFn(c.name)}() {
	case "$prev" in
...
function zshAction(v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return ":value:_pi_tools";
		case "models":
			return v.multiple ? ":models:_pi_models_list" : ":model:_pi_call models";
		case "sessions":
			return ":session:_pi_call sessions";
		case "file":
			return ":file:_files";
		case "dir":
			return ":dir:_files -/";
...
function generateZsh(spec: CompletionSpec): string {
	const { bin } = spec;
	// The `:value:_pi_tools` action references this helper; bake its candidates
...
${cmdRows}
	...
	const aliasArms = spec.commands
		.map(c => `\t\t\t${commandTokens(c).join("|")}) _pi_cmd_${bashFn(c.name)} ;;`)
		.join("\n");
	const rootSpecs = [
...
# Works both ways: autoloaded from $fpath (file named _pi) or eval'd from a
# startup file. When autoloaded, funcstack[1] is _pi and we invoke it; when
...
function generateFish(spec: CompletionSpec): string {
	...
	lines.push(`function __fish_pi_no_subcommand`);
	...
	const rootCond = "__fish_pi_no_subcommand";
