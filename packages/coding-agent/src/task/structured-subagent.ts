...
export async function resolveEffectiveSubagentPolicy(
	...
	return {
		...
		// The Task surface retains Pi's manual capture-only isolation contract.
		// Eval agent() owns the explicit apply control and keeps its prior default.
		applyChanges: request.invocationKind === "eval" && request.isolation?.apply !== false,
		enableLsp:
			!planMode &&
...
export async function runStructuredSubagent(request: StructuredSubagentRequest): Promise<StructuredSubagentResult> {
	...
	try {
		...
		if (
			...
			if (request.invocationKind === "task") mergeSummary = describeManualIsolation(result);
		...
		const shouldRetainArtifacts =
			...
			(policy.isIsolated &&
				((!policy.applyChanges && completedSuccessfully) || changesApplied === false || requiresRecoveryArtifacts));
