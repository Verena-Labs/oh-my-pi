...
import { aggregateUltraWorkerTokensPerSecond } from "../ultra/runtime";
...
export class InteractiveMode implements InteractiveModeContext {
	...
	constructor(
		...
		// Ultra worker tok/s aggregator — keeps the status-line render layer off
		// the heavy Ultra/task dependency graph. The orchestrator is often idle while
		// workers stream, so without this the tok/s badge would show a stale
		// value while parallel work is actively generating tokens.
		this.statusLine.setUltraWorkerTokenRateProvider(() =>
			aggregateUltraWorkerTokensPerSecond(this.session.getAgentId() ?? "main"),
		);
