...
export class StatusLineComponent implements Component {
	 ...
	 * live Ultra worker sessions, or null when no workers are streaming. Kept as
	 * a callback so the render layer doesn't import the heavy Ultra/task
	 * dependency graph; interactive-mode wires it to UltraSessionRegistry.
	 */
	#ultraWorkerTokenRate: (() => number | null) | null = null;
	 ...
	 * live Ultra worker sessions (null when no workers are streaming). Wired by
	 * interactive-mode, which owns the UltraSessionRegistry coupling, so the
	 * render layer stays off the heavy Ultra/task dependency graph. Pass
	 * `undefined` to clear.
	 */
	setUltraWorkerTokenRateProvider(provider: (() => number | null) | undefined): void {
		this.#ultraWorkerTokenRate = provider ?? null;
	}
	...
	#getTokensPerSecond(): number | null {
		// Aggregate tok/s across the main session AND every live Ultra worker.
		// During Ultra orchestration the parent is often idle while workers stream, so the
		// main session's own rate alone would show a stale/zero value while
		// parallel work is actively generating tokens.
		const workerRate = this.#getUltraWorkerTokensPerSecond();
		if (workerRate !== null) {
	 ...
	 * message timestamp. Preserves the pre-aggregation behavior when no Ultra
	 * workers are active.
	 */
	...
	/**
	 * Aggregate tok/s across every live Ultra worker session owned by this
	 ...
	 * injected via {@link setUltraWorkerTokenRateProvider} to keep this render
	 * layer off the heavy Ultra/task dependency graph.
	 */
	#getUltraWorkerTokensPerSecond(): number | null {
		return this.#ultraWorkerTokenRate?.() ?? null;
	}
