						...
						if (role === "default") {
							...
							if (
								...
								if (resolved.model) {
									...
									let configuredThinking = resolved.thinkingLevel;
									let concreteThinking = concreteThinkingLevel(configuredThinking);
									...
									if (!resolved.explicitThinkingLevel && !concreteThinking) {
										...
										if (defaultLevel === AUTO_THINKING) {
											...
											configuredThinking = defaultLevel;
											concreteThinking = concreteThinkingLevel(defaultLevel);
										}
									}
									const effectiveIsAuto = isAuto || isAutoFromDefault;
									const effectiveIsUltra = configuredThinking === ULTRA_THINKING;
									const { switched } = await this.ctx.session.setModel(fallbackModel, "default", {
										persist: false,
										thinkingLevel:
											effectiveIsAuto || effectiveIsUltra
									...
									if (effectiveIsAuto) {
										this.ctx.session.setThinkingLevel(AUTO_THINKING, true);
									} else if (effectiveIsUltra) {
										this.ctx.session.setThinkingLevel(ULTRA_THINKING, true);
									} else if (concreteThinking && concreteThinking !== ThinkingLevel.Inherit) {
										this.ctx.session.setThinkingLevel(concreteThinking);
									}
									this.ctx.statusLine.invalidate();
