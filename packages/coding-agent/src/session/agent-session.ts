...
import { createInterruptedTurnAbortMessage } from "./exit-diagnostics";
import {
...
export interface AgentSessionDisposeOptions {
	...
	/** Postmortem reason that triggered this dispose. */
	reason?: import("@oh-my-pi/pi-utils").postmortem.Reason;
...
export class AgentSession {
	...
	async refreshMCPTools(mcpTools: CustomTool[]): Promise<void> {
		...
		const nextActive = [...new Set([...this.#getActiveNonMCPToolNames(), ...allowedMcpTools.map(tool => tool.name)])];
		try {
