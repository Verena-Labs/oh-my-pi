...
async function registerPersistedSubagentsFromDir(
	...
	for (const entry of entries) {
		...
		if (!registry.get(id)) {
			...
			const workerKind = persisted?.init?.workerKind;
			registry.register({
				id,
				...
				session: null,
				sessionFile,
				// Ultra workers belong to a live owning orchestration session. An
				// unclaimed transcript discovered after restart is historical only.
				status: workerKind === "ultra" ? "aborted" : "parked",
			});
		}
