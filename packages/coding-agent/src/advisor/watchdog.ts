 ...
 * `.pi/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.pi/<F> (or the agent dir supplied by the caller)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .pi/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .pi folder, and user agent dir.
