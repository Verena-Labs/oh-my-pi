...
- `path` — required. Local path, internal URI (`local://`, `agent://`, `artifact://`, `history://`, `mcp://`, `memory://`, `skill://`, `vault://`, `ssh://`, `pi://`), or URL. Inline `:<sel>` still works for ranges/modes (e.g. `src/foo.ts:50-200`, `src/foo.ts:raw`, `db.sqlite:users:42`).
...
- Reader-mode default: HTTP(S) HTML, JSON endpoints, feeds, and PDFs → clean text/markdown when possible.
