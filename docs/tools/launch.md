...
> Launch and control long-running project processes shared by every Pi instance in the same directory.
...
- Pi process presence: `packages/coding-agent/src/launch/presence.ts`
...
`detached: true` implies `persist: true`, forces `pty: false`, and disables stdin. Its process survives broker shutdown and every Pi exit; a later broker reconnects to its records for logs and explicit `stop`.
...
Every Pi session registers its process in the canonical project scope. The first `launch` call starts a detached broker over a private socket; later `launch` calls from any registered Pi process connect to the same broker and see the same names, logs, and state.

Runtime data lives under `~/.pi/run/daemons/<project-hash>/`:
- `broker.sock` (or a Windows named pipe)
- a mode-0600 authentication token
- broker PID metadata
- per-managed-process launch metadata and logs
- live Pi process-presence records

After the last tool socket disconnects, the broker checks the project-presence records. Live Pi PIDs keep non-persistent managed processes running even when those Pi instances have not called `launch`; dead PIDs are removed. Once no Pi process remains, the broker waits three seconds, stops every non-persistent managed process, and exits. This PID check still works when a Pi process is killed without JavaScript cleanup.

`persist: true` explicitly opts a managed process out of last-client teardown. A broker with a live persistent process remains available without clients until another Pi reconnects and stops it. Broker recovery terminates stale recorded children and preserves their records as exited instead of adopting an unknown process state.
