...
| 10 | 11 | For a model with controllable reasoning effort, selecting the Ultra composite thinking tier leaves the main agent on its current model, requests `xhigh` clamped to that model's highest supported effort, retains its complete pre-Ultra active toolset, and adds exactly the five `ultra_*` orchestration tools. Each direct Ultra worker snapshots the main model and resolved effort when spawned through one generic full-capability worker definition outside the named task-agent catalog, and every descendant inherits that root snapshot. Existing worker trees stay pinned, while later direct spawns use the main agent's newly selected model. Sessions continue and steer, queue follow-ups safely, wait/cancel, remain owner-isolated, and render in stable spawn order with accurate state, turn, model, activity, and output snapshots in the existing live wall and Agent Hub. | Enter and leave Ultra through every existing thinking-level control and session-resume path, including a main tool call while workers run, multiple concurrent workers, a main-model change that proves existing worker trees remain pinned and later direct spawns use the new model, a nested spawn that proves descendant inheritance, viewport changes, interruption, worker death, and cleanup on exit. Prove Ultra is unavailable when reasoning effort is not controllable and that a lower maximum effort clamps consistently for main and workers. The spawn schema has no tier, model, CLI, or named-agent selector. `/ultra`, `/delegate`, `/vibe`, `delegate_*`, and `vibe_*` must be absent from commands, help, completion, callable tools, persistence restoration, and transcript-specific render compatibility. Ordinary `xhigh`, the `slow` role, and named `task` agents must remain behaviorally distinct. |
...
| 49 | 67 | Features and agents resolve configured roles such as default, smol, slow, plan, vision, designer, commit, tiny, task, and advisor without hard-coded model IDs; new work adopts changes while an active turn remains stable. | Use distinguishable fixture models. Role routing is separate from the provider fast/service tier, and Ultra must not appear as a model role or change the independently assigned `slow` model. |
