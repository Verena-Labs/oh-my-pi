...
| 10 | 11 | Delegate Mode starts persistent fast/good workers, continues and steers them, queues input safely, waits/cancels, and requires the director to verify evidence before proceeding. Its native live TV wall renders every owned worker in stable spawn order with accurate state, turn, activity, and output snapshots throughout the run. | Exercise `/delegate`, the `delegate_*` tools, multiple concurrent workers, viewport changes, interruption, worker death, and mode exit; `/vibe` and `vibe_*` must be absent, while the wall and Agent Hub agree on shared worker state without leaking another owner's sessions. |
...
| 52 | 74 | Session and global service tier, sampling, reasoning effort, verbosity, and provider-specific controls map exactly to supported outbound fields, persist at the advertised scope, and clear cleanly. | Capture requests for supported APIs; unsupported combinations must fail without approximate mutation. |
